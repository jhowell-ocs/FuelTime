...
def check_wkhtmltopdf():
    ...
    try:
        ...
        print(
            f"✓ wkhtmltopdf found: {result.stdout.split()[1] if len(result.stdout.split()) > 1 else 'version unknown'}"
        )
        return True
...
def version():
    ...
    return jsonify({"version": __version__, "app": "FuelTime", "organization": "Obion County Schools"})
    ...
    try:
        ...
        if WKHTMLTOPDF_INSTALLED:
            try:
                ...
                test_options = {"page-size": "Letter", "quiet": None, "encoding": "UTF-8"}
...
def preview_pdf():
    ...
    try:
        ...
        # Return PDF file
        return send_file(
            pdf_path, as_attachment=True, download_name=f"{formatted_filename}.pdf", mimetype="application/pdf"
        )
...
def submit_timesheet():
    ...
    try:
        ...
        return jsonify({"success": False, "message": f"Error generating timesheet: {str(e)}"}), 500
...
def preview_timesheet_pdf():
    ...
    try:
        ...
        # Debug logging
        print("Debug: Processing timesheet data for total hours calculation")
        ...
        return jsonify({"success": False, "message": f"Error generating timesheet PDF: {str(e)}"}), 500
